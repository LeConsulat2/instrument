...
def save_stems(output_folder, original_file_name):
    ...
    for stem in stem_names:
        ...
        if os.path.exists(stem_file_path):
            ...
            # Apply additional filtering to the "other" category
            if stem == "other":
                further_filter_other_stem(
                    stem_file_path,
                    os.path.join(output_folder, f"{base_name}-filtered-{stem}.wav"),
                )


# Additional Filtering for "Other" Stem
def further_filter_other_stem(input_file, output_file):
    # Load the "other" stem
    rate, data = read(input_file)

    # Ensure data is in the correct format for noise reduction
    if data.ndim > 1:
    ...
    # Save the filtered "other" stem
    write(output_file, rate, reduced_noise.astype(np.int16))
    print(f"Further filtered 'other' stem saved as {output_file}")
